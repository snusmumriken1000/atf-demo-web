...
面 3(Ask)では、このサイトの内容について質問できる。**外部の AI API は使っていない。**
[`src/lib/data/content.ts`](src/lib/data/content.ts) を知識源にした全文検索で答えるため、
外部通信はゼロで、オフラインでも動く。

- **知識源は content.ts だけ**。文言を差し替えればボットの回答も自動で変わる。書かれていないことは
  推測せず「答えられない」と返し、答えられる範囲を提示する
- **日本語の分かち書きは文字バイグラム**(外部の形態素解析器を持ち込まないため)。
  関連度は BM25 で測る
- **意図判定**で「経歴は?」→ 経歴を全件、「使える技術は?」→ スキルを全分類、のようにまとめて答える。
  意図が分からない質問は全文検索の上位で答える
- 回答には**その内容がある場所へのリンク**(`/profile#work-xxx` など)を添える
- 会話ログは `role="log"` + `aria-live="polite"`。質問例はボタンなのでキーボードだけで会話できる

| ファイル                      | 役割                                                      |
| ----------------------------- | --------------------------------------------------------- |
| `src/lib/bot/tokenize.ts`     | 分かち書き(英数字は語、日本語は文字バイグラム)            |
| `src/lib/bot/knowledge.ts`    | content.ts から検索単位(経歴・スキル・作品・リンク)を組む |
| `src/lib/bot/retrieve.ts`     | BM25 による関連度スコアリング                             |
| `src/lib/bot/intents.ts`      | 質問の意図判定(手がかり語)                                |
| `src/lib/bot/answer.ts`       | 回答文・出典・次に聞けることの組み立て                    |
| `src/routes/ask/+page.svelte` | 会話ログと入力欄                                          |
...
入口・Showcase・Profile・Ask の表示へ反映される(面 3 のボットの回答もここから作られる)。現在値はすべてサンプル。
...
5. `ask` で面 3 の挨拶・質問例・回答の前置きを調整する
6. `npm run verify` で型、表示テスト、静的出力、外部リソース非依存を確認する
...
検出する。さらに Playwright Chromium で全 4 ルートを開き、遅延表示後までのネットワーク通信が
...
- `src/lib/bot/` — 面 3 の対話ボット(知識の組み立て・検索・回答生成)
